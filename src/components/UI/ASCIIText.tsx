...
class AsciiFilter {
  ...
  hue() {
    // Disabled hue rotation to minimize hover effect as requested
    /*
...
class CanvAscii {
  ...
  setMesh() {
    ...
    // We will initially create a 1x1 geometry and scale it dynamically in setSize
  ...
  setSize(w: number, h: number) {
    ...
    if (this.mesh && this.textCanvas) {
      ...
      const textAspect = this.textCanvas.width / this.textCanvas.height;
      let targetW, targetH;
      if (textAspect > this.camera.aspect) {
        targetW = visibleWidth * 0.9; // 90% width
        targetH = targetW / textAspect;
      } else {
        targetH = visibleHeight * 0.9; // 90% height
        targetW = targetH * textAspect;
      }

      this.mesh.scale.set(targetW, targetH, 1);
    }
  }
  ...
  onMouseMove(evt: MouseEvent | TouchEvent) {
    const e =
      'touches' in evt ? (evt as TouchEvent).touches[0] : (evt as MouseEvent);
  ...
  updateRotation() {
    // Reduced tilt range from 0.5 to 0.02 to make the 3D rotation extremely subtle
    const x = mapRange(this.mouse.y, 0, this.height, 0.02, -0.02);
    const y = mapRange(this.mouse.x, 0, this.width, -0.02, 0.02);
