...
const ProjectCard = forwardRef<HTMLDivElement, ProjectCardProps>(
  function ProjectCard({ project }, ref) {
    ...
    return (
      <MagicCard className="cursor-target project-card group relative flex flex-col overflow-hidden rounded-xl border border-[var(--color-border-muted)] bg-[var(--color-background-alt)] transition-all duration-300 hover:border-[var(--color-border-accent)] hover:-translate-y-2 hover:shadow-xl hover:shadow-[var(--color-shadow-accent)] focus-within:border-[var(--color-border-accent)] focus-within:-translate-y-2 focus-within:shadow-xl focus-within:shadow-[var(--color-shadow-accent)]">
        <div ref={setRefs} className="contents">
          {/* ── Image Area ── */}
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                {project.repoUrl && (
                  <a
                    ...
                    onClick={() =>
                      trackProjectClick(project.title, project.repoUrl)
                    }
